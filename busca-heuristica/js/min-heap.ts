...
export class MinHeap<Element> {
    private heap: Element[];
    private elementValueGetter: (element: Element) => number;

    constructor (elementValueGetter: (element: Element) => number) {
        this.heap = [null]
    ...
    includes(element: Element) {
        return this.heap.includes(element);
    }

    find(predicate: (value: Element, index: number, obj: Element[]) => unknown) {
